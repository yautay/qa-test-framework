...
- Marker inventory helper: run `.venv/bin/python tools/pytest/generate_marker_matrix.py` to regenerate `docs/MARKER_TESTS_MATRIX.md` (marker -> test mapping). Refresh it after marker/test taxonomy changes.
- Repository tree helper: run `.venv/bin/python tools/docs/generate_repo_tree_html.py` to regenerate `docs/REPO_TREE.html` (collapsible HTML tree for whole repo). Refresh it after major structural changes.
