...
- Marker inventory helper: run `.venv/bin/python tools/pytest/generate_marker_matrix.py` to regenerate `docs/MARKER_TESTS_MATRIX.md` and `docs/MARKER_TESTS_TREE.md` (marker <-> test mapping and tree view). Use `--sort-by marker|test` for tree ordering. Refresh after marker/test taxonomy changes.
