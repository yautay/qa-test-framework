...
def test_search_listings_exceptions(page, runtime_env, admin_panel, url_suffix: str):
    ...
    entries = AdminSettingsExceptionListPage(page, admin_panel.admin_env.base_url).navigate_to().get_category_entries(limit=4)
    ...
    for entry in entries:
        ...
        assert count > 0, (
            f"Listing kategorii z wyjątku '{entry.config_name}' (id={entry.category_id}) jest pusty dla suffixu '{url_suffix}'."
        )
        ...
        assert system_codes, f"Listing kategorii id={entry.category_id} nie zwrócił żadnych kodów systemowych produktów."
        assert product_names, f"Listing kategorii id={entry.category_id} nie zwrócił żadnych nazw produktów."
