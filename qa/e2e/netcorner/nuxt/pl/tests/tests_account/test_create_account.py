...
from qa.e2e.netcorner.nuxt.pl.lib.wrapers.register_client import RegisterClient
...
def test_create_account(page, context, runtime_env, user_data):
    assert RegisterClient(page, context, runtime_env).register_new_client(user_data)