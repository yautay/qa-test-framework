...
def test_baseline_events_are_persisted_in_state_json(tmp_path: Path) -> None:
    repo_root = tmp_path / "repo"
    repo_root.mkdir()
    run_id = "run-baseline-events"
    report_dir = repo_root / "artifacts" / run_id / "visual"
    report_dir.mkdir(parents=True)
    (report_dir / ".report-ready.json").write_text('{"ready": true}\n', encoding="utf-8")
    (report_dir / "results.json").write_text(
    ...
    try:
        ...
        assert status == 200
        assert payload["accepted"] is True
        assert payload["test_cases"][case_id]["baseline"] is True

        status, payload = _http_json(base_url, f"/api/builds/{run_id}/tags")
        assert status == 200
        assert payload["tags"]["test_cases"][case_id]["baseline"] is True
        ...
        assert status == 200
        assert payload["test_cases"][case_id]["baseline"] is False
