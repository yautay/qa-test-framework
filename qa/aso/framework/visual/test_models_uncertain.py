...
class TestVisualThresholdsToDict:
    def test_includes_uncertain_deltas(self):
        t = VisualThresholds(
            pixel_max=0.005,
            lpips_max=0.08,
            dists_max=0.08,
        ...
        d = t.to_dict()
        assert d["pixel_uncertain_delta"] == 0.002
        assert d["lpips_uncertain_delta"] == 0.015
        assert d["dists_uncertain_delta"] is None
