...
def load_env() -> RuntimeEnv:
    ...
    settings_headless = bool(getattr(settings_cli, "is_session_headless", True))
    settings_grid_available = bool(
        getattr(settings_cli, "is_grid_available", getattr(settings, "is_grid_available", False))
    )
    settings_grid_ws_endpoint = str(
        getattr(settings_cli, "grid_ws_endpoint",
    ...
    configured_browser = env_value("BROWSER",
                                   getattr(settings_cli, "browser", "chromium")) or "chromium"
    browser = configured_browser.strip().lower()
    reporting_source_origin = env_value("REPORTING_SOURCE_ORIGIN",
                                        getattr(settings_cli, "reporting_source_origin", "")) or ""
    ...
    return RuntimeEnv(
        ...
        grid_ws_endpoint=env_value("GRID_WS_ENDPOINT",
                                   settings_grid_ws_endpoint) or settings_grid_ws_endpoint,
        grid_connect_timeout_ms=int(
        ...
        ignore_https_errors=_as_bool(env_value("IGNORE_HTTPS_ERRORS"),
                                     settings_cli.ignore_https_errors),
        base_url=base_url,
        server_type=server_type,
        server_name=server_name,
        record_video=_as_bool(env_value("RECORD_VIDEO"), True),
        video_min_seconds=int(env_value("VIDEO_MIN_SECONDS", "30") or "30"),
        reporting_enabled=_as_bool(env_value("REPORTING_ENABLED"), settings_cli.reporting_enabled),
        reporting_schema_version=env_value(
            "REPORTING_SCHEMA_VERSION",
            getattr(settings_cli, "reporting_schema_version", "2.0"),
        )
                                 or "2.0",
        reporting_source_project=env_value(
            "REPORTING_SOURCE_PROJECT",
            getattr(settings_cli, "reporting_source_project", Path.cwd().name),
        )
                                 or Path.cwd().name,
        reporting_source_origin=reporting_source_origin,
        framework_version=env_value(
            "FRAMEWORK_VERSION",
            getattr(settings_cli, "framework_version", "1.0.0"),
        )
                          or "1.0.0",
        reporting_api_url=env_value("REPORTING_API_URL", settings_cli.reporting_api_url) or "",
        reporting_api_token=env_value("REPORTING_API_TOKEN",
                                      settings_cli.reporting_api_token) or "",
        reporting_api_run_start_endpoint=env_value(
            "REPORTING_API_RUN_START_ENDPOINT",
            settings_cli.reporting_api_run_start_endpoint,
        )
                                         or settings_cli.reporting_api_run_start_endpoint,
        reporting_api_test_result_endpoint=env_value(
            "REPORTING_API_TEST_RESULT_ENDPOINT",
            settings_cli.reporting_api_test_result_endpoint,
        )
                                           or settings_cli.reporting_api_test_result_endpoint,
        reporting_api_run_finish_endpoint=env_value(
            "REPORTING_API_RUN_FINISH_ENDPOINT",
            settings_cli.reporting_api_run_finish_endpoint,
        )
                                          or settings_cli.reporting_api_run_finish_endpoint,
        reporting_api_timeout_seconds=int(
            env_value("REPORTING_API_TIMEOUT_SECONDS",
                      str(settings_cli.reporting_api_timeout_seconds))
            or str(settings_cli.reporting_api_timeout_seconds)
        ),
        reporting_api_retries=int(
            env_value("REPORTING_API_RETRIES", str(settings_cli.reporting_api_retries))
            or str(settings_cli.reporting_api_retries)
        ),
        ...
        visual_enabled=_as_bool(
            ...
            bool(getattr(settings_cli, "visual_enabled",
                         getattr(settings, "visual_enabled", False))),
        ),
        visual_compare_mode=(
                env_value(
                    "VISUAL_COMPARE_MODE",
                    str(getattr(settings_cli, "visual_compare_mode",
                                getattr(settings, "visual_compare_mode", "hybrid"))),
                )
                or "hybrid"
        ).strip().lower(),
        visual_baseline_provider=(
                env_value(
                    "VISUAL_BASELINE_PROVIDER",
                    str(getattr(settings_cli, "visual_baseline_provider",
                                getattr(settings, "visual_baseline_provider", "minio"))),
                )
                or "minio"
        ).strip().lower(),
        visual_baseline_profile=env_value(
            "VISUAL_BASELINE_PROFILE",
            str(getattr(settings_cli, "visual_baseline_profile",
                        getattr(settings, "visual_baseline_profile", "test-ref"))),
        )
                                or "test-ref",
        visual_baseline_version=env_value(
            "VISUAL_BASELINE_VERSION",
            str(getattr(settings_cli, "visual_baseline_version",
                        getattr(settings, "visual_baseline_version", "latest"))),
        )
                                or "latest",
        visual_cache_dir=env_value(
            "VISUAL_CACHE_DIR",
            str(getattr(settings_cli, "visual_cache_dir",
                        getattr(settings, "visual_cache_dir", ".visual_cache"))),
        )
                         or ".visual_cache",
        ...
        visual_warn_as_fail=_as_bool(
            ...
            bool(getattr(settings_cli, "visual_warn_as_fail",
                         getattr(settings, "visual_warn_as_fail", False))),
        ),
        visual_minio_endpoint=env_value(
            "VISUAL_MINIO_ENDPOINT",
            str(getattr(settings_cli, "visual_minio_endpoint",
                        getattr(settings, "visual_minio_endpoint", ""))),
        )
                              or "",
        visual_minio_access_key=env_value(
            "VISUAL_MINIO_ACCESS_KEY",
            str(getattr(settings_cli, "visual_minio_access_key",
                        getattr(settings, "visual_minio_access_key", ""))),
        )
                                or "",
        visual_minio_secret_key=env_value(
            "VISUAL_MINIO_SECRET_KEY",
            str(getattr(settings_cli, "visual_minio_secret_key",
                        getattr(settings, "visual_minio_secret_key", ""))),
        )
                                or "",
        visual_minio_bucket=env_value(
            "VISUAL_MINIO_BUCKET",
            str(getattr(settings_cli, "visual_minio_bucket",
                        getattr(settings, "visual_minio_bucket", "visual-baselines"))),
        )
                            or "visual-baselines",
        visual_minio_secure=_as_bool(
            env_value("VISUAL_MINIO_SECURE"),
            bool(getattr(settings_cli, "visual_minio_secure",
                         getattr(settings, "visual_minio_secure", True))),
        ),
        ...
        visual_perceptual_api_url=env_value(
            ...
            str(getattr(settings_cli, "visual_perceptual_api_url",
                        getattr(settings, "visual_perceptual_api_url", ""))),
        )
                                  or "",
        visual_perceptual_timeout_seconds=int(
            env_value(
