...
export async function sendSingleAttempt(runId, key, type, noteHash = null, options = {}) {
  ...
  try {
    const response = await fetch(`/api/reports/${id}/report/send-attempt`, {
      method: "POST",
    ...
    if (!response.ok) {
      throw new Error(payload?.error || "unable to send attempt");
    }
...
export async function retryFailedAttempts(runId, options = {}) {
  const id = encodeURIComponent(String(runId || "").trim());
  const timeoutMs = typeof options?.timeoutMs === "number" ? options.timeoutMs : 30000;
  const controller = new AbortController();
  const timeoutId = globalThis.setTimeout(() => controller.abort(), timeoutMs);
  try {
    const response = await fetch(`/api/reports/${id}/report/retry-failed`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({}),
      signal: controller.signal,
    ...
    const payload = await parseJsonResponse(response);
    if (!response.ok) {
      throw new Error(payload?.error || "unable to retry failed");
    }
    return payload && typeof payload === "object" ? payload : {};
  } catch (error) {
    if (error?.name === "AbortError" || error instanceof TypeError) {
