...
2. Build suite map from all payload scenarios.
3. Detect shared Arrange and shared flow fragments across scenarios.
4. Design one coherent architecture:
   - test module(s)
...
5. Decide what to reuse vs extend in POM/flows/builders.
6. Implement tests in Arrange-Act-Assert shape with explicit business assertions per scenario.
7. Keep domain split strict: no business assertions in POM/flow methods.
8. Verify markers/imports/constants align with local conventions.
