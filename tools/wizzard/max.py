...
from tools.wizzard.locators import *
import subprocess
...
def parse_wizard() -> list:
    ...
    inputs_data = []
    for el in elements:
        input_id = el.get_attribute("id").split('-')[0]
...
def check_vm_crontab(ssh_host: str, ssh_port: str) -> bool:
    ...
    if os.name == 'nt':  # Windows
        cmd = ("sshpass -p {nc_pass} ssh -o StrictHostKeyChecking=no -p {server_ssh_port} {host} "
               r'crontab -l | grep -vE "^\s*(#|$)"').format(nc_pass=nc_container_pass, host=ssh_host, server_ssh_port=ssh_port)
        command = [
            "wsl",
            "bash",
            "-c",
...
def check_vm_mock(ssh_host: str, ssh_port: str) -> bool:
    remote_cmd = "grep -m1 \"'enabled'\" /srv/http/netcorner/config/config.d/mock.php | sed -E \"s/.*'enabled' => ([a-z]+).*/\\1/i\""
    command = [
        "sshpass",
        "-p", nc_container_pass,
        "ssh -o StrictHostKeyChecking=no",
    ...
    if os.name == 'nt':  # Windows
        cmd = ("sshpass -p {nc_pass} ssh -o StrictHostKeyChecking=no -p {server_ssh_port} {host} {rmt_cmd}").format(
            nc_pass=nc_container_pass, host=ssh_host, server_ssh_port=ssh_port, rmt_cmd=remote_cmd)
        command = [
            "wsl",
            "bash",
            "-c",
